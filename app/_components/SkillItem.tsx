...
function SkillItem({ skill, title }: SkillItemProps) {
  return (
    <div className="w-[400px] capitalize flex flex-col text-center items-center tracking-wider rounded-lg duration-75">
      ...
      <ul className="flex flex-col items-center justify-center gap-2 py-4 text-[#fff] w-full">
        {skill?.map(({ id, name }) => (
          <li key={id} className="flex w-full gap-2 justify-center">
            <div className="text-center text-primary dark:text-secondary text-lg">
              {name}
            </div>
